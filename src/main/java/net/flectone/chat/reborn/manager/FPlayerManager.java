...
public class FPlayerManager {
    ...
    public void loadTabCompleteData() {
        FlectoneChatReborn.getPlugin().getDatabase().execute(connection -> {
    ...
    public FPlayer getOffline(@Nullable String name) {
        ...
        if (offlinePlayerList.size() > 1) {
            FlectoneChatReborn.warning("Please remove offline players with the same names, it can cause problems");
        }
