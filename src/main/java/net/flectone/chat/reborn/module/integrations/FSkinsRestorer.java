...
public class FSkinsRestorer implements FIntegration {
    ...
    private void firstHook() {
        try {
            ...
            FlectoneChatReborn.info("SkinsRestorer detected and hooked");
        } catch (Exception e) {
            FlectoneChatReborn.warning("SkinsRestorer is not initialized yet. This is due to proxy. The second hook attempt will be in 10 seconds");
            secondHook();
        }
    }

    private void secondHook() {
        Bukkit.getScheduler().runTaskLaterAsynchronously(FlectoneChatReborn.getPlugin(), () -> {
            try {
                skinsRestorer = SkinsRestorerProvider.get();
                FlectoneChatReborn.info("SkinsRestorer detected and hooked");
            } catch (Exception e) {
                FlectoneChatReborn.warning("SkinsRestorer is not initialized after all");
            }
        }, 200L);
    }
