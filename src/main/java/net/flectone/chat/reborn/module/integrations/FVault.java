...
public class FVault implements FIntegration {
    ...
    @Override
    public void init() {
        RegisteredServiceProvider<Chat> chatProvider = FlectoneChatReborn.getPlugin().getServer().getServicesManager()
                .getRegistration(net.milkbowl.vault.chat.Chat.class);
        RegisteredServiceProvider<Permission> permissionProvider = FlectoneChatReborn.getPlugin().getServer().getServicesManager()
                .getRegistration(net.milkbowl.vault.permission.Permission.class);

        if (chatProvider == null || permissionProvider == null) {
            FlectoneChatReborn.warning("Failed to load Vault Chat, you may not have LuckPerms installed");
            return;
        }
        ...
        FlectoneChatReborn.info("Vault detected and hooked");
    }
