...
import net.flectone.chat.reborn.model.mail.Mail;
...
public class Database extends SQLHandler {
    ...
    public Database(FlectoneChatReborn plugin) {
        super(plugin.getDataFolder().getAbsolutePath());

        config = FlectoneChatReborn.getPlugin().getFileManager().getConfig();
    ...
    public void onConnect() {
        FlectoneChatReborn.info("SQLite DB Connected successfully");
        init();
    }
    ...
    public void init() {
        try {
            ...
            if (FlectoneChatReborn.getPlugin().getFileManager().isLess420()) {
                migrate420();
            }

            if (FlectoneChatReborn.getPlugin().getFileManager().isLess450()) {
                migrate450();
            }
    ...
    public void migrate420() {
        try (Connection conn = getConnection()) {
            ...
            FlectoneChatReborn.warning("Couldn't execute MySQL statement: " + ex);
        }
    }
    ...
    public void updateSettings(@NotNull FPlayer fPlayer, @NotNull String column) {
        ...
        try (Connection conn = getConnection()) {
            ...
            FlectoneChatReborn.warning( "Couldn't execute SQLite statement: ");
            ex.printStackTrace();
        }
    }
